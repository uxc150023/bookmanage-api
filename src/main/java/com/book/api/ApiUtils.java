...
import com.book.util.SqlInj;
import com.book.validator.MyValidator;
import com.book.annotation.*;
    ...
    public static boolean checkUpdateParams(Object object) throws ApiException {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if (value instanceof String) {
                String newValue = ((String) value).replace(";", "；").replace("+", "").replace("\'", "“").replace(",", "，");
                if (!SqlInj.sql_inj((String) newValue)) {
            ...
            if(java.util.List.class.isAssignableFrom(typeClass)){
                if(value != null){
                    ...
                    if(list.size() >0){
                        ...
                        if(notEmpty != null){
                            String _item = "";
                            throw new ApiException(notEmpty.subCode(), _item + notEmpty.message());
                        }
                    }
                }
                else {
                    NotNull notNull = fields[i].getAnnotation(NotNull.class);
                    if(notNull != null){
                        String _item = "";
                        throw new ApiException(notNull.subCode(), _item + notNull.message());
                    }
...
//      String _item = itemName + ":" + value + ",";
            ...
            if (NotNull.class.isAssignableFrom(typeClass)) {
                ...
                if (!MyValidator.isLength(length.min(), length.max(), "" + value)) {
                    throw new ApiException(length.subCode(), _item + length.message());
                }
                ...
                if (!MyValidator.isMax(max.value(), Long.parseLong("" + value))) {
                ...
                if (!MyValidator.isMin(min.value(), Long.parseLong("" + value))) {
                ...
                if (!MyValidator.isMobile((String) value)) {
                    throw new ApiException(mobile.subCode(), _item + mobile.message());
                }
                ...
                if (!MyValidator.isPassword((String) value)) {
                    throw new ApiException(password.subCode(), _item + password.message());
                }
